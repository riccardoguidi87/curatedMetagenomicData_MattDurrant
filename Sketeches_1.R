...
# now that I have the list of studies with controls, I'll check the first

df %>%
  group_by(study_name,study_condition) %>%
  summarise(count = n()) %>%
  filter(study_name == studyNamesWithCtr[5])

# the list worked!
# now find CRC studies with ctr
...
df %>%
  filter(study_name == studiesCRCwithCtr$study_name[1]) %>% view

# FengQ_2015 is the first study with CRC and internal Ctrs. Although there is no annotation
# in the curatedMetagenomic table, the original article seem to have some
# I see if I can pull the table and merge

mdFengQ_2015_SupplData1 <- read_excel("raw-data/FengQ_2015_PMID25758642/41467_2015_BFncomms7528_MOESM1193_ESM.xlsx")
...
# Data vidualization and exploration
...
violin_fun(data = mdFengQ_2015_SupplData1_3,
...
all_plots2$Clinical_data_State
...
colleciton_plots2$Clinical_data_State
